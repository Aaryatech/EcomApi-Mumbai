...
public interface GetFlavorTagStatusListRepo extends JpaRepository<GetFlavorTagStatusList, Integer> {
	...
	@Query(value = " SELECT m_filter.filter_id,m_filter.filter_name,m_filter.filter_type_id,m_filter.used_for_filter, " + 
			" m_filter.cost_affect,m_filter.used_for_description,m_filter.company_id,m_filter.sort_no " + 
			" FROM m_filter  " + 
			" WHERE m_filter.filter_type_id IN (4,7,5) and m_filter.del_status=1 AND m_filter.is_active=1 AND m_filter.company_id=:companyId " + 
