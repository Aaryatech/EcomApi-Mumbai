...
public interface MFilterRepo extends JpaRepository<MFilter, Integer> {
	...
	@Query(value="SELECT\n" + 
			...
			"    m_filter_type t\n" + 
			"WHERE\n" + 
			"    f.filter_type_id = t.filter_type_id AND f.del_status = 1 AND t.del_status = 1 AND t.is_active = 1 AND f.company_id = :compId\n" + 
			"ORDER BY\n" + 
			"    f.filter_id\n" + 
			"DESC\n" + 
			"    ", nativeQuery=true)
	List<MFilter> getAllFilters(@Param("compId") int compId);
