...
public interface CategorywiseSellRepo  extends JpaRepository<CategorywiseSell, Integer>{
	...
	@Query(value = "SELECT\n" + 
			...
			"    m_category\n" + 
			"WHERE\n" + 
			"    t_sell_bill_header.sell_bill_no = t_sell_bill_detail.sell_bill_no AND t_sell_bill_header.bill_date BETWEEN :fromDate AND :toDate AND t_sell_bill_detail.del_status = 1 AND t_sell_bill_header.del_status = 1 AND t_sell_bill_header.fr_id =:frId AND m_category.cat_id = t_sell_bill_detail.cat_id\n" + 
			"GROUP BY\n" + 
