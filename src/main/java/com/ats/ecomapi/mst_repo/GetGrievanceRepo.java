...
public interface GetGrievanceRepo extends JpaRepository<GetGrievanceHeader, Integer> {
	...
	@Query(value="SELECT\n" + 
			...
			"    LEFT JOIN\n" + 
			"        m_user user \n" + 
			"            ON     t_grievance.insert_by_id = user.user_id \n" + 
			"    WHERE\n" + 
			"        t_grievance.order_id = tn_order_header.order_id \n" + 
			"        AND      t_grievance.grievance_type_id = mn_grievences_type_instructn.grev_type_id \n" + 
			"        AND     t_grievance.grievance_subtype_id = mn_grievences_instruction.grievance_id \n" + 
			"        AND     m_customer.cust_id = tn_order_header.cust_id \n" + 
			"        AND     m_franchise.fr_id = tn_order_header.fr_id \n" + 
			"        AND     tn_order_header.order_delivered_by=:delBoyId",nativeQuery=true)
	List<GetGrievanceHeader> getGrievanceHeaderByDelBoyId(@Param("delBoyId") int delBoyId );
